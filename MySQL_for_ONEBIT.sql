# upraveno pro MySQL na hostingu onebit
...
WITH country_key as (
    ...
    (
    SELECT CASE
        WHEN MAX(lt.lat) > 0 AND DAYOFYEAR(cd.date) BETWEEN 1 AND 80 THEN 0
        WHEN MAX(lt.lat) > 0 AND DAYOFYEAR(cd.date) BETWEEN 81 AND 172 THEN 1
        WHEN MAX(lt.lat) > 0 AND DAYOFYEAR(cd.date) BETWEEN 173 AND 266 THEN 2
        WHEN MAX(lt.lat) > 0 AND DAYOFYEAR(cd.date) BETWEEN 267 AND 355 THEN 3
        WHEN MAX(lt.lat) > 0 AND DAYOFYEAR(cd.date) BETWEEN 356 AND 366 THEN 0
        WHEN MAX(lt.lat) <= 0 AND DAYOFYEAR(cd.date) BETWEEN 1 AND 80 THEN 2
        WHEN MAX(lt.lat) <= 0 AND DAYOFYEAR(cd.date) BETWEEN 81 AND 172 THEN 3
        WHEN MAX(lt.lat) <= 0 AND DAYOFYEAR(cd.date) BETWEEN 173 AND 266 THEN 0
        WHEN MAX(lt.lat) <= 0 AND DAYOFYEAR(cd.date) BETWEEN 267 AND 355 THEN 1
        WHEN MAX(lt.lat) <= 0 AND DAYOFYEAR(cd.date) BETWEEN 356 AND 366 THEN 2
    END
